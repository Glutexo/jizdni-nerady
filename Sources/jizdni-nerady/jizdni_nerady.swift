...
struct CommandRunner {
    ...
    private func connectionsOutput(for arguments: [String]) async throws -> String {
        ...
        guard let from = options.value(for: "--from", short: "-f"), !from.isEmpty else {
            throw CommandError.usage("Usage: jizdni-nerady connections --from place --to place [--timetable alias] [--date d.m.yyyy] [--time h:mm] [--direct] [--format text|markdown|json] [--limit count]")
        }
    ...
    private var helpText: String {
        ...
        🚆 Usage:
          ...
          jizdni-nerady connections --from place --to place [--timetable alias] [--date d.m.yyyy] [--time h:mm] [--direct] [--format text|markdown|json] [--limit count]
